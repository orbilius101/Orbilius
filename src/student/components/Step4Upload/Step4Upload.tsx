        ...
        <Stack spacing={4}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            <Box sx={{ width: 52, height: 52, borderRadius: '50%', bgcolor: '#ffd700', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <Typography sx={{ color: 'background.default', fontWeight: 700, fontSize: '1.4rem', lineHeight: 1 }}>4</Typography>
            </Box>
          ...
          {status === 'Submitted' || status === 'Approved' ? (
            ...
            <Paper
              ...
              onClick={() => { if (!justDroppedRef.current) document.getElementById('file-input-s4')?.click(); }}
              ...
              onDrop={handleDrop}
            >
              <input id="file-input-s4" type="file" accept="application/pdf" onChange={handleFileChange} style={{ display: 'none' }} />
          ...
          {file && status !== 'Submitted' && status !== 'Approved' && (
            ...
            >
              {uploading ? 'Uploading...' : 'Upload Implementation Docs'}
            </Button>
          )}
